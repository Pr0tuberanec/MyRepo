...
<         self.hyper_cell = nn.LSTMCell(hidden_size + input_size, hyper_size, bias=bias)
< 
...
<         self.z_h = nn.Linear(hyper_size, 4 * n_z, bias=False)
<         self.z_x = nn.Linear(hyper_size, 4 * n_z, bias=False)
